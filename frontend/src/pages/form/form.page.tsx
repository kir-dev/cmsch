...
const FormPage = () => {
  ...
  const { mutate: submit, isPending: submitLoading, data: result } = useFormSubmit(params.slug || '')
  ...
  const available = form && form.availableFrom * 1000 < now && form.availableUntil * 1000 > now && !detailsValidated
  ...
  const onSubmit = (values: Record<string, unknown>) => {
    ...
    if (available) {
      submit({ formBody: newValues, edit: status !== FormStatus.NO_SUBMISSION })
      window.scrollTo(0, 0)
    }
  }
